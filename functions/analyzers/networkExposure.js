...
function analyzeNetworkExposure(files) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            for (const pattern of CRITICAL_PATTERNS) {
                ...
                if (thisFileIsWebhook) {
                    // Webhook / bot receiver — network binding is expected
                    mediumCount++;
                    findings.push({
                        severity: "medium",
                        file: file.path,
                    ...
                    // Auth detected in this file or sibling files
                    lowCount++;
                    findings.push({
                        severity: "low",
                        file: file.path,
                    ...
                    // No auth, not a webhook — genuinely dangerous
                    criticalCount++;
                    findings.push({
                        severity: "critical",
                        file: file.path,
            ...
            /* ── Warning patterns (env/config host) — unchanged ── */
            ...
            for (const pattern of URL_PATTERNS) {
                if (pattern.regex.test(line)) {
                    if (pattern.severity === "high") {
                        // We use lowCount to track these so they don't break the build automatically
                        // Wait, the audit says HTTP to external should be flagged. Let's use medium.
                        mediumCount++;
                    } else {
                        lowCount++;
                    }
            ...
            /* ── Safe patterns — just mark server code ── */
    ...
    /* ════════════════════════════════════════════════
       Scoring — severity-weighted, not boolean
       ════════════════════════════════════════════════ */
    let score;
    let status;

    if (criticalCount > 0) {
        // Criticals dominate — heavy penalty
        score = Math.max(0, 40 - criticalCount * 20);
        status = "fail";
    } else if (mediumCount > 0) {
        // Medium only (webhooks / env host) — moderate penalty
        score = Math.max(50, 90 - mediumCount * 5);
        status = mediumCount >= 3 ? "warn" : "warn";
    } else if (lowCount > 0) {
        // Auth detected — minimal penalty
        score = Math.max(75, 95 - lowCount * 5);
        status = "pass";
    } else if (hasServerCode) {
        score = 100;
        status = "pass";
    } else {
        // No server binding — stdio transport (safe)
        score = 100;
        status = "pass";
    }
